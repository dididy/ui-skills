...
| CSS transition can't do multi-step (A→B hold→C) | CSS transition only supports A→B. Chaining is fragile | Use WAAPI `element.animate()` with multi-keyframe + `offset`. Or `@beyond/core animateSequence()` (WAAPI with hold support) |
| Animation code changes don't take effect | Modified shared package (`@beyond/core`) but consuming app has stale build | `pnpm --filter <pkg> build` → restart dev server. Turbopack/Next.js dev does NOT hot-reload package `dist` changes |
