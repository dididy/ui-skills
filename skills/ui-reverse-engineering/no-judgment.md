...
| "GSAP Premium — I'll approximate" | See `transition-implementation.md`: SplitText→`splitting`, MorphSVG→`flubber`, ScrollSmoother→`lenis`, DrawSVG→`stroke-dashoffset`. |
...
| "DOM section wrapper probably isn't needed" | `agent-browser eval "document.querySelector('.section').children[0].className"` — CSS sets margin/padding ON the wrapper. |
